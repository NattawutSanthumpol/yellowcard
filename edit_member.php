...
if ($up == "updateData") {
	...
	if ($_POST["mem_name"] != "" and $_POST["mem_username"] != "") {
		...
		$strSQL   = "UPDATE member SET mem_name = '$mem_name', mem_user = '$mem_username', mem_pass = '$mem_pass', lvl_id = '$mem_lvl' WHERE mem_id = '$mem_id'";
	...
	<div class="container"><br>
		...
		<div class="row justify-content-md-center">
			<div class="col-auto form-group">
				<form action="edit_member.php" method="POST" enctype="multipart/form-data" name="form1">
					<table border="1" class="table table-hover table-responsive">
						...
						<tbody>
							<tr>
								...
								<td><input type="text" class="form-control" name="mem_id" value="<?= $res['mem_id'] ?>" readonly></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><input type="text" class="form-control" name="mem_username" value="<?= $res['mem_user'] ?>"></td>
							</tr>
							<tr>
								<td align="right">Password : </td>
								<td><input type="text" class="form-control" name="mem_pass" value="<?= base64_decode($res['mem_pass']) ?>"></td>
							</tr>
							<tr>
								...
								<td>
									<select name="mem_lvl" class="form-control">
										...
										while ($showlvl = mysqli_fetch_array($querySQL,MYSQLI_ASSOC)) {
											?>
											<option value="<?= $showlvl['lvl_id'] ?>" <?php
												if ($showlvl['lvl_id'] == $res['lvl_id']) {
													echo "selected";
												} else {
													echo "";
												}
												?>
											>
												<?= $showlvl['lvl_name'] ?></option>
										<?php } ?>
									</select>
								</td>
							</tr>
							<tr>
								<td align="center" colspan="2"><button type="submit" class="btn btn-success" name="UpDateData" value="updateData">แก้ไขข้อมูล</button>
								</td>
							</tr>
						</tbody>
					</table>
