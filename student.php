 ...
 */
$strSQL   = "SELECT * FROM student order by LENGTH(std_class), std_class ASC "; //LIMIT 10
				...
				<table border="1" class="table table-hover table-responsive">
					...
					<tbody>
						<?php while ($objResult = mysqli_fetch_array($objQuery,MYSQLI_ASSOC)) { ?>
							<tr>
								<th scope="row" class="text-center"><?= $objResult["std_id"] ?></th>
								<td><?= $objResult["std_tname"] ?></td>
								<td><?= $objResult["std_fname"] ?></td>
								<td><?= $objResult["std_lname"] ?></td>
								<td><?= $objResult["std_class"] ?></td>
								<td><?= $objResult["std_depart"] ?></td>
							</tr>
						<?php } ?>
					</tbody>
				</table>
