 ...
 */
$dateSec ;
isset($_POST["search"]) ? $dateSec = $_POST["search"] : $dateSec = "" ;
$se;
isset($_POST["se"]) ? $se = $_POST["se"] : $se = "";
if ($se == "") {
	...
	$strSQL   = "SELECT * FROM std_offense LEFT JOIN student ON std_offense.std_id = student.std_id LEFT JOIN wrong ON std_offense.wr_id = wrong.wr_id LEFT JOIN member ON std_offense.mem_id = member.mem_id WHERE std_offense.std_off_date LIKE '" . $dateSec . "%' AND std_offense.std_off_status = 'ติดกิจการ'  ORDER BY std_off_date DESC";
}
...
<body>
	...
	<div class="container">
		<p>
		<form action="add_std_off.php" method="POST" enctype="multipart/form-data" name="form1">
			<button type="submit" class="btn btn-success">เพิ่มข้อมูล</button>
		</form>
		<!-- ค้นหาข้อมูล -->
		<form action="chkview.php" method="POST" enctype="multipart/form-data" name="form2">
			<table class="table table-borderless table-responsive">
				<tr>
					<td align="right" valign="bottom">ค้นหาวันที่ : </td>
					<td width="80%"><input class="form-control" type="date" id="example-date-input" name="search"></td>
					<td align="left"><button type="submit" class="btn btn-success" name="se" value="search">ค้นหา</button></td>
				</tr>
				<!-- แสดงผล -->
			</table>
		</form>
		<table border="0" class="table table-hover table-responsive col-sm-auto col-md-auto">
			<thead class="thead-dark">
				<tr align="center">
					...
					<?php if ($_SESSION['Status'] == 1 or $_SESSION['Status'] == 2) { ?>
			...
			<tbody>
				...
				while ($res = mysqli_fetch_array($objQuery, MYSQLI_ASSOC)) { ?>
					<tr>
						...
						<?php if ($_SESSION['Status'] == 1 or $_SESSION['Status'] == 2) { ?>
							<td align="center"><a href="update_std_off.php?id=<?= $res['std_off_id'] ?>" class="btn btn-warning btn-sm click" onclick="chk()">แก้ไขสถานะ</a></td>
							<script>
								$(function chk() {
									//$("a").confirm({
									$(".click").confirm({
